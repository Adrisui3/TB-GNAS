...
class SearchSpace:
    ...
    def learn_for_depth(self, model_list: list, depth: int, positive: bool):
        with self.lock:
            for lay_act, block in zip(model_list, self.space[depth]):
                block.learn(layer=lay_act[0], activation=lay_act[1], positive=positive)

    def query_for_depth(self, depth: int) -> list:
        with self.lock:
            model = []
